...
algorithm and a packaging version, tracking changes individually between the algorithm logic and the packaging of it.
...
== Glossary
The following terms are specific to Seed and SHOULD be referenced to clarify their meaning throughout the specification.
...
|executor
|process responsible for injecting all run-time context: inputs, mounts and environment settings
