...
:docinfo:
:doctype: manpage
:manmanual: Seed Specification
:mansource: Seed Specification

== Name

seed-spec - A general standard to aid in the discovery and consumption of a discrete unit of work contained within a Docker image.

== Introduction

While initially developed to support the http://ngageoint.github.io/scale/[Scale] data processing system with job
discovery, the seed specification is designed to be readily applied to other systems as well.
...
== Standard
include::sections/standard.man.adoc[]

== Usage
include::sections/usage.man.adoc[]

== Discovery
include::sections/discovery.man.adoc[]
...
*executor* ::
    process responsible for injecting run-time context (inputs, mounts and environment settings) and capturing all resulting output (files, json).
