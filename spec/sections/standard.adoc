...
.Files Elements
Below are the File elements for outputs.
...
.JSON Elements
Below are the JSON elements for outputs.
...
|`category`
|Optional
2+|MUST be a string containing one of the following values: `job`, `data` or `system`. If omitted, the default
value is `job`.
...
<4> Optional string containing the error type. This value MUST be either: `job`, `data` or `system`. The default
value is `job`.
|=====
