...
===== Interface Object
The Interface object is the primary member that describes the arguments, environment variables,
...
complete parameter string passed to the executable defined in the docker image's ENTRYPOINT. Based on the Linux shell, shell escaping
...
----
{
    "args": "${INPUT_FILE} ${JOB_OUTPUT_DIR}", // <1>
...
<4> Optional <<envvars-section>>. This is a means to set environment variables before a job is run.
<5> Optional <<mounts-section>>. This defines any directories that need to be mounted into the job container.
...
the container at job execution time in the `JOB_OUTPUT_DIR` environment variable.
