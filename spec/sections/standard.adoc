...
.Scalar Elements
The Scalar objects MAY include any arbitrary custom resource name, but there are reserved resources `cpu`, `mem`, and
`disk` that have special meaning and SHOULD be populated by all Seed compliant images.

* The Scalar object MUST have a member `name`. The member's value MUST be a string indicating the resource required by
the job.
* The Scalar object MUST have a member `value`. The member's value MUST be a float indicating the quantity of the
resource required by the job. When dealing with `mem` and `disk` reserved
* The Scalar object MAY have a member `inputMultiplier`. The member's value MUST be a float indicating the factor by
which input data size (in MiBs) is multiplied and added to the constant value for resource.
...
For example, when input data volume is 2.0MiB and an `inputMultiplier` of `4.0` and `value` of `0.1` is specified for
`disk`, the following computes the resource requirement:
...
----
<1> Recommended Scalar element demonstrating single constant scalar value for specify CPU requirement of job.
<2> Recommended Scalar element demonstrating single constant scalar value in addition to a multiplier based on input volume
size for scaling disk requirement of job.
<3> Recommended Scalar element demonstrating single constant scalar value in addition to a multiplier based on input volume
size for scaling memory requirement of job.
