...
<<environment-variables,environment variables>>. Below are the File elements for inputs.
...
.JSON Elements
Below are the JSON elements for inputs.
...
.Files Object
The following table defines the `ouputs.files` object members.
...
.JSON Object
The following table defines the `ouputs.json` object members.
...
|`type`
|Required
2+|MUST be a string indicating the JSON schema type of the member being captured from the result manifest.

|`key`
|Optional
2+|MUST be a string indicating the case-sensitive result manifest member to capture. If omitted, the result member
