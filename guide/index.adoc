...
[float]
==== How do I use Docker?
Plenty of resources for learning about Docker can be found at https://www.docker.com. The primary goal of this guide 
is to explain how to integrate an algorithm with Seed and not focus on how to build a Docker container.
...
These "Settings" can be marked as "secret" and will be stored in https://www.vaultproject.io/[HashiCorp Vault]. The values 
for these environment variables are hidden from the API and are only injected at runtime.
