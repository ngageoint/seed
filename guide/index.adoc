...
3. The algorithm outputs some information about the output to the command line and to a file.
4. The algorithm saves the rotated image as a new file in the specified output directory.
5. We wrote a simple shell script to launch our algorithm.
6. We wrote a basic Dockerfile that identified a base image, installed dependencies, and copied our algorithm and launch script into it.
7. We built a Docker image of our own and called it `img-test`.
8. Finally, we ran a Docker container from our `img-test` image, mounted a volume, and passed it the command to run our algorithm with the required positional parameters.
...
4. We used our `rotate_image.sh` script to call our Python code and is a common pattern when writing Dockerfiles.
...
4. `${BYTE_COUNT}`. The Seed specification contract ensures that this variable will be populated with an integer value to the input since we have given it the explicit type "integer" and marked it as a required input.
