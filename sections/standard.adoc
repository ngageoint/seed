...
======= Files Elements
The Files elements contain the objects responsible for indicating file content that will be made available within a
JobPack container at runtime.
...
======= JSON Elements
The JSON elements is the member responsible for indicating file content that will be made available within a JobPack
container at runtime.

* The Files object MUST have a member `name`. The member's value MUST be a string indicating the environment
variable name that will be injected by the processing platform for job consumption.
* The Files object MUST have a member `type`. The member's value MUST be a string and indicate a valid JSON schema type.
* The Files object MAY have a member `required`. The member's value MUST be a boolean indicating whether this input
value SHOULD always be expected. If omitted, the default value MUST be treated as true.
...
*** The Content object MUST have a member `mediaType`. The member's value MUST indicate the IANA Media type for
the file being captured by OutputData.
...
is true.
<4> Required string (only for `file` type) containing IANA Media type on `file` type output.
