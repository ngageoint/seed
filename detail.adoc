...
----
include::examples/complete/jobpack.json[]
----
...
=== Implementing
A few requirements must be satisfied when implementing a system capable of executing JobPack standardized images.
...
* All `name` member values of `interface.inputData.file` elements: Indicate the absolute path of input files.
environment variable name case MUST map exactly from `name` value.
...
<3> Required string containing an ISO 8601 date indicating the start temporal extent of file.
<3> Required string containing an ISO 8601 date indicating the end temporal extent of file. If the data is a single
point-in-time the end date should match the start date.
...
----
include::examples/random-number/jobpack.json[]
----
...
==== Image Watermark Job
Image watermark job taking a single image and returning with watermark applied using JobPack definition.
----
include::examples/watermark/jobpack.json[]
----
...
offerings and the following sections describe the steps that may be taken to access the labels defined by JobPack.
...
`-jobpack`. This allows for quick filtering of results to a manageable set for discovery.
...
** URL: https://hub.docker.com/v2/search/repositories/?query=-jobpack
* Perform HTTP POST to get label details for images found in previous request:
** BODY: {"repos":[{"name":"myorg/myjob-jobpack","tag":"latest"}]}
...
The following steps may be taken to find and identify labels of JobPack compliant images within Docker Registry:

NOTE: All references to `{registry}`, `{image-id}` and `{tag}` in the following URLs should be replaced with your
environment specific values.

* Perform HTTP GET against catalog endpoint to find `-jobpack` suffixed images:
